...
def find_mhd_file(patient_id):
    for subject_no in range(settings.LUNA_SUBSET_START_INDEX, 10):
        src_dir = settings.LUNA16_RAW_SRC_DIR + "subset" + str(subject_no) + "\\" + "subset" + str(subject_no) + "\\"
...
def process_pos_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\luna16_annotations\\annotations.csv")
    ...
    for index, annotation in df_patient.iterrows():
        ...
        pos_annos.append([anno_index, numpy.round(center_float_percent[0], 4), numpy.round(center_float_percent[1], 4), numpy.round(center_float_percent[2], 4), numpy.round(diameter_percent, 4), 1])
        anno_index += 1

    df_annos = pandas.DataFrame(pos_annos, columns=["anno_index", "coord_x", "coord_y", "coord_z", "diameter", "malscore"])
    df_annos.to_csv(settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels\\" + patient_id + "\\"+ patient_id + "_annos_pos.csv", index=False)
    return [patient_id, spacing[0], spacing[1], spacing[2]]


def process_excluded_annotations_patient(src_path, patient_id):
    df_node = pandas.read_csv(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\luna16_annotations\\annotations_excluded.csv")
    dst_dir = settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels\\"
    if not os.path.exists(dst_dir):
        os.mkdir(dst_dir)
    dst_dir = dst_dir + patient_id + "\\"
    ...
    pos_annos_df = pandas.read_csv(settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels\\" + patient_id +"\\"+ patient_id + "_annos_pos.csv")
    ...
    for index, annotation in df_patient.iterrows():
        ...
        neg_annos.append([anno_index, numpy.round(center_float_percent[0], 4), numpy.round(center_float_percent[1], 4), numpy.round(center_float_percent[2], 4), numpy.round(diameter_percent, 4), 1])
        anno_index += 1

    df_annos = pandas.DataFrame(neg_annos, columns=["anno_index", "coord_x", "coord_y", "coord_z", "diameter", "malscore"])
    df_annos.to_csv(settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels\\" + patient_id + "\\"+ patient_id + "_annos_excluded.csv", index=False)
    return [patient_id, spacing[0], spacing[1], spacing[2]]
...
def process_luna_candidates_patient(src_path, patient_id):
    ...
    src_df = pandas.read_csv(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\luna16_annotations\\candidates_V2.csv")
    ...
    df_candidates.to_csv(dst_dir + patient_id +"\\" +patient_id +"_candidates_luna.csv", index=False)
...
def process_auto_candidates_patient(src_path, patient_id, sample_count=1000, candidate_type="white"):
    ...
    df_candidates = pandas.DataFrame(candidate_list, columns=["anno_index", "coord_x", "coord_y", "coord_z", "diameter", "malscore"])
    df_candidates.to_csv(dst_dir + patient_id +"\\" + patient_id +"_candidates_" + candidate_type + ".csv", index=False)
...
def process_lidc_annotations(only_patient=None, agreement_threshold=0):
    ...
    neg_count = 0
    all_lines = []
    for anno_dir in [d for d in glob.glob(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\luna16_annotations\\*") if os.path.isdir(d)]:
