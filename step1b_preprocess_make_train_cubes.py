...
def make_pos_annotation_images():
    src_dir = settings.LUNA16_EXTRACTED_IMAGE_DIR + "metadata"
...
def make_annotation_images_lidc():
    ...
    for patient_index, csv_file in enumerate(glob.glob(src_dir + "*_annos_pos_lidc.csv")):
        ...
        if len(df_annos) == 0:
            continue
        pimages = helpers.load_patient_images(patient_id, settings.LUNA16_EXTRACTED_IMAGE_DIR, "*" + CUBE_IMGTYPE_SRC + ".png")

        for index, row in df_annos.iterrows():
            coord_x = int(row["coord_x"] * pimages.shape[2])
            coord_y = int(row["coord_y"] * pimages.shape[1])
            coord_z = int(row["coord_z"] * pimages.shape[0])
            ...
            cube_img = get_cube_from_img(pimages, coord_x, coord_y, coord_z, 64)
            if cube_img.sum() < 5:
                print(" ***** Skipping ", coord_x, coord_y, coord_z)
                continue
...
def make_pos_annotation_images_manual():
    src_dir = r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\luna16_manual_labels\\"
...
def make_candidate_auto_images(candidate_types=[]):
    ...
    for candidate_type in candidate_types:
        if candidate_type == "falsepos":
            src_dir = r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\luna16_falsepos_labels\\"
        else:
            src_dir = settings.LUNA16_EXTRACTED_IMAGE_DIR + "_labels\\"
...
def make_pos_annotation_images_manual_ndsb3():
    src_dir = r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\ndsb3_manual_labels\\"
    dst_dir = settings.BASE_DIR_SSD + "generated_traindata\\ndsb3_train_cubes_manual\\"
    if not os.path.exists(dst_dir):
        os.mkdir(dst_dir)


    train_label_df = pandas.read_csv(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\stage1_labels.csv")
