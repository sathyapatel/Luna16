...
def random_scale_img(img, xy_range, lock_xy=False):
    ...
    org_height = img[0].shape[:2]
    org_width = img[0].shape[:2]
...
def get_train_holdout_files(model_type, holdout, train_percentage=80, frame_count=8):
    ...
    file_paths = glob.glob(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\kaggle_ndsb2017\\kaggle_ndsb2017-master\\resources\\resources\\segmenter_traindata\\" + "*_1.png")
    file_paths.sort()
    train_res = []
    holdout_res = []
...
class DumpPredictions(Callback):
    ...
    def __init__(self, dump_filelist : List[Tuple[str, str]], model_type):
        ...
        self.batch_count = 0
        if not os.path.exists("E:\\stage1\\stage1\\segmenter\\"):
            os.mkdir("E:\\stage1\\stage1\\segmenter\\")
        for file_path in glob.glob("E:\\stage1\\stage1\\segmenter\\*.*"):
            os.remove(file_path)
...
def train_model(holdout, model_type, continue_from=None):
    ...
    checkpoint1 = ModelCheckpoint(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\trained_models\\" + model_type +"_model_h" + str(holdout) + "_{epoch:02d}-{val_loss:.2f}.hd5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
    checkpoint2 = ModelCheckpoint(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\trained_models\\" + model_type +"_model_h" + str(holdout) + "_best.hd5", monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=1)
    files = []
    idx = 0
    ...
    epoch_div = 1
    epoch_count = 10 if model_type == "masses" else 50
    ...
    if not os.path.exists("models"):
        os.mkdir("models")
    shutil.copy(r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\trained_models\\"+ model_type +"_model_h" + str(holdout) + "_best.hd5", r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\trained_models\\" + model_type +"_model_h" + str(holdout) + "_best.hd5")
    ...
    if True:
        for model_type_name in ["masses"]:
            ...
            for holdout_no in [0, 1, 2]:
                ...
                predict_patients(patients_dir=patient_base_dir, model_path= r"C:\\Users\\Sangryal\\Downloads\\sathya\\2nd place\\trained_models\\" + model_type_name + "_model_h" + str(holdout_no) + "_best.hd5", holdout=holdout_no, patient_predictions=patient_predictions_global, model_type=model_type_name)
